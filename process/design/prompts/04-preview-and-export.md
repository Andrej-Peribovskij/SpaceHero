...
| `DS_PACKAGE_NAME` | preview, **export** | `@space-hero/design-system` — what the frontend pins, installs and **imports**. The exporter's `--ds-package`: it matches imports by this name, so a frontend that aliases the design system (`"@space-hero/design-system": "npm:…"`) finds none of its components without it. **Not a substitute for `DS_REPO_PATH`** — `node_modules/@space-hero/design-system` ships `dist/` and structurally cannot serve §3 |

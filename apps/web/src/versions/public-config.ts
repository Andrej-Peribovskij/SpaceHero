 ...
 * THIS FILE IS THIS APP'S. It is written once and never copied over: it is the one place the
 * version machinery touches this project's API. Everything that reads it is process-owned.
 */
 ...
 *
 * See `docs/design-version-registry.md` §"The runtime contract" for what the endpoint must
 * answer, and `process/design/templates/version-registry/backend/` for a reference
 * implementation to copy into the API.
 */

/** The endpoint. Unauthenticated: it gates first paint, before there is a session to ask with. */
const PUBLIC_CONFIG_URL = "/api/v1/public-config";
